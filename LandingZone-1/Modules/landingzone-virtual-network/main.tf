...
resource "azurerm_resource_group" "spoke_networking_rg" {
  name     = var.resource_group_name
  location = var.location
}
...
resource "azurerm_virtual_network" "spoke_landingzone_vnet" {
  ...
  location            = var.location
  resource_group_name = azurerm_resource_group.spoke_networking_rg.name
...
resource "azurerm_network_security_group" "NSGs" {
  name                = "${var.landingzone_NSG_Prefix}_${each.key}"
  location            = azurerm_resource_group.spoke_networking_rg.location
  resource_group_name = azurerm_resource_group.spoke_networking_rg.name
}
